...
def test_cylindrical(engine_cylindrical: SupervisedMDS, X: np.typing.NDArray[np.float64],
                     city_coordinates: np.typing.NDArray[np.float64]) -> None:
    engine_cylindrical.fit(X, city_coordinates)
    X_proj = engine_cylindrical.transform(X)
    assert X_proj.shape == (10, 2)


def test_geodesic(engine_geodesic: SupervisedMDS, X: np.typing.NDArray[np.float64],
                  city_coordinates: np.typing.NDArray[np.float64]) -> None:
...
def test_spherical(engine_spherical: SupervisedMDS, X: np.typing.NDArray[np.float64],
                   city_coordinates: np.typing.NDArray[np.float64]) -> None:
