...
def create_plots(
    ...
    n_rows_bars = 2  # always 2 rows for bars
    n_cols_bars = math.ceil(n_metrics / n_rows_bars)
    total_cols = 1 + n_cols_bars

    width_ratios = [1.5] + [1] * n_cols_bars
    ...
    if primary_col in valid_df.columns:
        best_row = valid_df.sort_values(by=primary_col, ascending=False).iloc[0]
    else:
        best_row = valid_df.sort_values(by=metric_cols[0], ascending=False).iloc[0]

    best_shape_name = best_row["shape"]
    best_score_val = best_row.get(primary_col, 0.0)
    ...
    if best_shape_obj:
        ...
        try:
            ...
            title_str = f"Best Manifold: {best_shape_name}\n"
            title_str += f"({primary_metric.replace('_', ' ').title()}: {best_score_val:.4f})"
            ax_scatter.set_title(title_str, fontsize=15, fontweight="bold")
    ...
    for idx, col_name in enumerate(metric_cols):
        ...
        r = idx // n_cols_bars
        c = 1 + (idx % n_cols_bars)  # Offset by 1 because col 0 is scatter

        # Safety check if we have more metrics than expected
        if r >= n_rows_bars:
            break
        ...
        # Sort:
        # Assuming higher is better for plotting
