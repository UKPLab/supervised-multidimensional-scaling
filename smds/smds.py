...
class SupervisedMDS(BaseEstimator, TransformerMixin):
    def __init__(self, n_components: int = 2, manifold: Union[str, Callable] = 'trivial', 
                 alpha: float = 0.1, orthonormal: bool = False, radius: float = 6371):
        """
        Parameters:
            n_components:
    ...
    def _compute_ideal_distances(self, y: np.ndarray, threshold: int = 2) -> np.ndarray:
        ...
        if self.manifold in ['trivial', 'cluster']:  # Retrocompatibility
            ...
            if len(y.shape) != 2 or y.shape[1] != 2:
                raise ValueError("For 'sphere_chord', labels must be a 2D array " \
            ...
            if len(y.shape) != 2 or y.shape[1] != 2:
                raise ValueError("For 'geodesic', labels must be a 2D array " \
                "with shape (n_samples, 2).")
            radius = self.radius
            ...
            if len(y.shape) != 2 or y.shape[1] != 2:
                raise ValueError("For 'cylinder_chord', labels must be a 2D array " \
                "with shape (n_samples, 2).")
    ...
    def _masked_loss(self, W_flat: np.ndarray, X: np.ndarray, 
                     D: np.ndarray, mask: np.ndarray) -> float:
        """
