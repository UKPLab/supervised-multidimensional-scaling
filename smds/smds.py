...
class PrecomputedManifoldScaler(BaseEstimator, TransformerMixin):
    def __init__(
        self,
        n_components: int = 2,
        alpha: float = 0.1,
    ...
    def _least_squares(self, X: np.typing.NDArray) -> None:
        self._X_mean = X.mean(axis=0)  # Centering
        self._Y_mean = self.Y_.mean(axis=0)  # Centering Y
        X_centered = X - X.mean(axis=0)
        Y_centered = self.Y_ - self.Y_.mean(axis=0)
        if self.orthonormal:
    ...
    def _masked_loss(
        self, W_flat: np.typing.NDArray, X: np.typing.NDArray, D: np.typing.NDArray, mask: np.typing.NDArray
    ) -> float:
        """
    ...
    def _truncated_pinv(self, W: np.typing.NDArray, tol: float = 1e-5) -> np.typing.NDArray:
        U, S, VT = np.linalg.svd(W, full_matrices=False)
        S_inv = np.array([1 / s if s > tol else 0 for s in S])
        return VT.T @ np.diag(S_inv) @ U.T

    def _regularized_pinv(self, W: np.typing.NDArray, lambda_: float = 1e-5) -> np.typing.NDArray:
        return np.linalg.inv(W.T @ W + lambda_ * np.eye(W.shape[1])) @ W.T

    def inverse_transform(self, X_proj: np.typing.NDArray) -> np.typing.NDArray:
        """
    ...
    def fit_transform(self, X: np.typing.NDArray, y: np.typing.NDArray) -> np.typing.NDArray:
        """
    ...
    def score(
        ...
        D_ideal = np.linalg.norm(self.Y_[:, np.newaxis, :] - self.Y_[np.newaxis, :, :], axis=-1)
...
class SupervisedMDS(PrecomputedManifoldScaler):
    def __init__(
        self,
        n_components: int = 2,
    ...
    def _compute_ideal_distances(self, y: np.typing.NDArray, threshold: int = 2) -> np.typing.NDArray:
        """
    ...
    def _classical_mds(self, D: np.typing.NDArray) -> np.typing.NDArray:
        """
    ...
    def _optimize_W(self, X: np.typing.NDArray, D: np.typing.NDArray):
        mask = D >= 0
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        if np.any(D < 0):
            ...
            self._optimize_W(X, D)
        else:
            # Use classical MDS + closed-form least squares
            self.Y_ = self._classical_mds(D)
            self._least_squares(X)
